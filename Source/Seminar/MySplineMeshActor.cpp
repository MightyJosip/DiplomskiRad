...
void AMySplineMeshActor::BeginPlay()
{
	Super::BeginPlay();


	// this->SetActorLocation(FVector(0, 0, 5));
	int imageSize = 1024;
...
float AMySplineMeshActor::getMapHeightAtPoint(FVector2D point)
{
...
void AMySplineMeshActor::generateImage(float* image, const int imageSize, FVector2D startLocation)
{
	for (int32 y = 0; y < imageSize; y++)
	{
		for (int32 x = 0; x < imageSize; x++)
...
void AMySplineMeshActor::saveImage(float* image, const int imageSize, const FString fileName, bool markLowest)
{
...
void AMySplineMeshActor::generateCurvePoints(float* image, const int imageSize)
{
	int pos_x = startPosition.X;
	int pos_y = startPosition.Y;
...
FVector AMySplineMeshActor::getSplineLocationAtPosition(FVector2D position)
{
	return FVector(position.X, position.Y, this->getMapHeightAtPoint(position));
}

TArray<FVector> AMySplineMeshActor::createPointsAtSpacing()
{
...
void AMySplineMeshActor::destroyOldSpline(bool destroyMesh)
{
...
void AMySplineMeshActor::initializeCurve()
{
	SplineMeshComponents.Empty();
	for (int index = 0; index < positions.Num(); index++)
	{
		SplineComponent->AddSplinePoint(FVector(-57100 + 111 * positions[index].X, -57100 + 111 * positions[index].Y, 0), ESplineCoordinateSpace::Local);
	}
...
FVector AMySplineMeshActor::getUpVectorAtPoint(FVector2D point)
{
	UWorld* world{ this->GetWorld() };
	if (point.X == 0) point.X = 1;
	if (point.Y == 0) point.Y = 1;
...
void AMySplineMeshActor::drawSpline()
{
	if (!this->GetSplineMeshComponent())
	{
		return;
	}
	...
	int numberOfMeshes = (curveLength - fmod(curveLength, 100)) / 100;
	float scaleFactor = 100000 / this->GetSplineMeshComponent()->GetStaticMesh()->GetBoundingBox().GetSize().X;
	for (int index = 0; index < numberOfMeshes + 1; index++)
	{
		USplineMeshComponent* splineMeshComponent = NewObject<USplineMeshComponent>(this, USplineMeshComponent::StaticClass());
		SplineMeshComponents.Add(splineMeshComponent);
		splineMeshComponent->SetStaticMesh(this->GetSplineMeshComponent()->GetStaticMesh());

		FVector startLocation = SplineComponent->GetLocationAtDistanceAlongSpline(index * 100, ESplineCoordinateSpace::Local);
		FVector startTangent = SplineComponent->GetDirectionAtDistanceAlongSpline(index * 100, ESplineCoordinateSpace::Local);

		FVector endLocation = SplineComponent->GetLocationAtDistanceAlongSpline((index + 1) * 100, ESplineCoordinateSpace::Local);
		FVector endTangent = SplineComponent->GetDirectionAtDistanceAlongSpline((index + 1) * 100, ESplineCoordinateSpace::Local);
...
// Called every frame
void AMySplineMeshActor::Tick(float DeltaTime)
{
